...
assert.match(globalCss, /\.monitoring-status-row--neutral\s*\{/, 'configured/not-started monitoring status must have a neutral accent');
...
assert.match(tripDetailSurface, /monitoring-status-row/, 'trip detail monitoring section must render the advisory monitoring status row');
...
assert.match(tripDetailSurface, /candidate:[\s\S]*confirmed:[\s\S]*planned:[\s\S]*monitoring:[\s\S]*active:[\s\S]*completed:[\s\S]*cancelled:/, 'Status milestone must define a concise description for every lifecycle status and cancelled');
assert.match(tripDetailSurface, /const stepTooltipId = `\$\{milestoneId\}-\$\{step\.key\}`;/, 'Each lifecycle status must get its own tooltip id');
assert.match(tripDetailSurface, /aria-describedby=\{stepTooltipId\}/, 'Each status milestone step must connect its tooltip text for assistive tech');
assert.match(tripDetailSurface, /tabIndex=\{0\}/, 'Status milestone tooltip must be keyboard reachable, not hover-only');
assert.match(tripDetailSurface, /className="status-milestone-step-wrap"/, 'Each milestone status must wrap its own hover/focus target');
...
assert.match(globalCss, /\.status-milestone-step-wrap:hover \.status-milestone-tooltip,[\s\S]*?\.status-milestone-step-wrap:focus-within \.status-milestone-tooltip,[\s\S]*?\.status-milestone-step-wrap:focus \.status-milestone-tooltip\s*\{[\s\S]*?opacity:\s*1;/,
  'Each status milestone tooltip must appear on hover and keyboard focus');
...
assert.doesNotMatch(tripDetailSurface, /className="detail-context-strip"/, 'Trip detail header must not render a dedicated context row below the Status milestone');
...
assert.match(tripDetailSurface, /const monitoring = monitoringLabel\(trip\)/, 'Status milestone must derive Monitoring copy from the already-loaded trip data');
assert.match(tripDetailSurface, /className="status-milestone-monitoring"[\s\S]*?📡 Monitoring \{monitoring\}/, 'Status milestone must integrate Monitoring as a right-side badge, not a dedicated row');
assert.match(globalCss, /\.status-milestone-content\s*\{[\s\S]*?justify-content:\s*space-between;[\s\S]*?\}/, 'Status milestone must lay out lifecycle and Monitoring badge in the same row when space allows');
assert.match(globalCss, /\.status-milestone-monitoring\s*\{[\s\S]*?border-radius:\s*999px;[\s\S]*?white-space:\s*nowrap;[\s\S]*?\}/, 'Integrated Monitoring badge must be compact and pill-like');
