...
assert.match(dashboardSurface, /title=\{formatMonitoringPhaseTooltip\(monitoringPhase\.phase\)\}/, 'summary monitoring chip must preserve a desktop tooltip fallback');
assert.match(dashboardSurface, /<details className="monitoring-phase-help">/, 'summary monitoring chip must expose the phase legend on tap for mobile Safari');
assert.doesNotMatch(dashboardSurface, /monitoring-phase-chip-icon/, 'summary monitoring chip should not render the old emoji icon');
assert.doesNotMatch(dashboardSurface, /monitoring-phase-chip-info/, 'summary monitoring chip should not render the old info emoji');
assert.match(dashboardSurface, /monitoring-phase-chip-label/, 'summary monitoring chip must render the compact phase label');
assert.match(tripDetailSurface, /title=\{formatMonitoringPhaseTooltip\(monitoringPhase\.phase\)\}/, 'detail monitoring chip must preserve a desktop tooltip fallback');
assert.match(tripDetailSurface, /<details className="monitoring-phase-help">/, 'detail monitoring chip must expose the phase legend on tap for mobile Safari');
assert.doesNotMatch(tripDetailSurface, /monitoring-phase-chip-icon/, 'detail monitoring chip should not render the old emoji icon');
assert.doesNotMatch(tripDetailSurface, /monitoring-phase-chip-info/, 'detail monitoring chip should not render the old info emoji');
