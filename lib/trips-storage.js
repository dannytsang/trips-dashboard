...
export function getMissingBlobStorageEnvironment(env = process.env) {
  return hasBlobStorageEnvironment(env) ? [] : ['BLOB_READ_WRITE_TOKEN'];
}
...
export async function readTripsDashboardPortfolio({
  ...
  try {
    ...
    throw new TripsPortfolioStorageError('Failed to read trips portfolio manifest from Blob storage', {
...
async function readExistingManifest(blobGet, pathname, env = process.env) {
  try {
    ...
    throw new TripsPortfolioStorageError('Failed to inspect current trips portfolio manifest', {
      code: 'storage_read_failed',
      cause: error,
    });
  }
