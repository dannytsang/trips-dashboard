...
export class TripsBriefStorageError extends Error {
  constructor(message, { code = 'storage_error', cause } = {}) {
    super(message);
    this.name = 'TripsBriefStorageError';
    this.code = code;
    this.cause = cause;
  }
}
...
export function assertBlobStorageConfigured(env = process.env) {
  ...
  if (missing.length > 0) {
    throw new TripsBriefStorageError(
...
function emptyRead(pathname, message = 'No trips brief has been synced yet.') {
  return {
...
export async function readTripsDashboardBrief({
  blobGet = get,
  env = process.env,
  ...
  try {
    ...
    throw new TripsBriefStorageError('Failed to read trips brief manifest from Blob storage', {
  ...
  try {
    ...
    for (const entry of manifest.trips || []) {
      ...
      if (!tripRead) {
        throw new Error(`Missing trip brief object ${entry.path}`);
      }
      trips[entry.id] = tripRead.value;
    }
    const normalised = validateTripsBriefEnvelope({ manifest, trips }, {
    ...
    throw new TripsBriefStorageError('Stored trips brief is invalid', {
      code: 'stored_brief_invalid',
      cause: error,
    });
  }
}
...
async function readExistingManifest(blobGet, pathname) {
  try {
    ...
    throw new TripsBriefStorageError('Failed to inspect current trips brief manifest', {
      code: 'storage_read_failed',
      cause: error,
    });
  }
...
async function readExistingChecksum(blobGet, sidecarPath) {
  try {
    ...
    if (isNotFoundResult(error)) return null;
    throw new TripsBriefStorageError('Failed to inspect current trip checksum sidecar', {
      code: 'storage_read_failed',
      cause: error,
    });
  }
...
export async function writeTripsDashboardBrief(briefInput, {
  blobGet = get,
  blobPut = put,
  ...
  const normalised = validateTripsBriefEnvelope(briefInput, { receivedAt: now });
  ...
  try {
    ...
    return {
      ...
      storage: {
        configured: true,
        exists: true,
        replacedExistingBrief: Boolean(existingManifest),
    ...
    throw new TripsBriefStorageError(
      'Failed to write trips brief to Blob storage; last known good brief was left intact',
      {
