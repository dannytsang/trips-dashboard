...
import {
  getDemoTripById,
  getDemoTripsDashboardMode,
  getDemoTripsDashboardPortfolio,
} from './trips-demo-fixtures.js';
...
export function resolveTripsDashboardMode(env = process.env) {
  if (hasBlobStorageEnvironment(env)) {
    return {
      mode: 'production',
      isDemo: false,
      dataSourceLabel: 'Private Blob storage',
      bannerMessage: null,
    };
  }

  return getDemoTripsDashboardMode();
}
...
export async function readTripsDashboardPortfolio({
  ...
  const mode = resolveTripsDashboardMode(env);
  if (mode.isDemo) {
  ...
  try {
    for (const existingEntry of existingManifest?.trips || []) {
      if (!desiredIds.has(existingEntry.id)) {
        removedTripIds.push(existingEntry.id);
        deletedPaths.push(...await deleteIfAny(blobDelete, [existingEntry.path, existingEntry.sidecarPath], env));
      }
    }
    ...
    for (const entry of desiredEntries) {
      ...
      if (existingChecksum) {
        deletedPaths.push(...await deleteIfAny(blobDelete, [entry.path, entry.sidecarPath], env));
      }
...
export async function readTripById(
  tripId,
  { blobGet = get, env = process.env, pathname = TRIPS_MANIFEST_BLOB_PATH } = {},
) {
  if (!hasBlobStorageEnvironment(env)) {
    return getDemoTripById(tripId);
  }
  ...
  const manifestEntry = (manifestRead?.value?.trips || []).find(entry => entry?.id === tripId);
  if (!manifestEntry?.path) {
    return null;
  }

  try {
    const read = await readJsonBlob(blobGet, manifestEntry.path, env);
    return read?.value ?? null;
  } catch (error) {
    if (isNotFoundResult(error)) {
      return null;
    }
    throw new TripsPortfolioStorageError(storageReadFailureMessage(`object ${manifestEntry.path}`, error),
