...
export const TRIPS_PORTFOLIO_SCHEMA_VERSION = 1;
...
function validateTripSummary(trip, index) {
  ...
  if (trip.schemaVersion != null && trip.schemaVersion !== TRIPS_PORTFOLIO_SCHEMA_VERSION) {
    errors.push(`${path}.schemaVersion must be ${TRIPS_PORTFOLIO_SCHEMA_VERSION}`);
  }
...
function validateManifest(manifest) {
  ...
  if (manifest.schemaVersion !== TRIPS_PORTFOLIO_SCHEMA_VERSION) {
    errors.push(`manifest.schemaVersion must be ${TRIPS_PORTFOLIO_SCHEMA_VERSION}`);
  }
...
export function unwrapPortfolioPayload(payload) {
...
export function validateTripsPortfolio(input, { receivedAt = new Date().toISOString() } = {}) {
  const brief = unwrapPortfolioPayload(input);
  const errors = [];

  if (!brief || typeof brief !== 'object' || Array.isArray(brief)) {
    throw new PortfolioValidationError('Brief payload must be an object', ['$ must be an object']);
  }
  ...
  if (brief.schemaVersion != null && brief.schemaVersion !== TRIPS_PORTFOLIO_SCHEMA_VERSION) {
    errors.push(`schemaVersion must be ${TRIPS_PORTFOLIO_SCHEMA_VERSION} when provided`);
  }
  ...
  if (byteLength(brief) > MAX_PORTFOLIO_BYTES) {
    errors.push(`brief must be ${MAX_PORTFOLIO_BYTES} bytes or smaller`);
  }

  if (errors.length > 0) {
    throw new PortfolioValidationError('Invalid trips brief', errors);
  }

  return {
    schemaVersion: TRIPS_PORTFOLIO_SCHEMA_VERSION,
...
export function validateTripsPortfolioEnvelope(input, { receivedAt = new Date().toISOString() } = {}) {
  const errors = [];
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new PortfolioValidationError('Brief envelope must be an object', ['$ must be an object']);
  }
  ...
  if (byteLength(input) > MAX_PORTFOLIO_BYTES) {
    errors.push(`brief envelope must be ${MAX_PORTFOLIO_BYTES} bytes or smaller`);
  }
...
export function createEmptyPortfolio({ generatedAt = null, receivedAt = null } = {}) {
  return {
    schemaVersion: TRIPS_PORTFOLIO_SCHEMA_VERSION,
    generatedAt,
    receivedAt,
