...
const THRESHOLDS = [
  { phase: 'fifteen_minute', limitMs: 60 * 60 * 1000, detail: 'Current phase: Fifteen minute' },
  { phase: 'hourly', limitMs: 4 * 60 * 60 * 1000, detail: 'Current phase: Hourly' },
  { phase: 'four_hourly', limitMs: DAY_MS, detail: 'Current phase: Four hourly' },
  { phase: 'daily_precheck', limitMs: 7 * DAY_MS, detail: 'Current phase: Daily precheck' },
];
...
const MONITORING_PHASE_LEGEND = [
  ...
  { phase: 'daily_precheck', label: 'Daily precheck', detail: 'Current phase: Daily precheck' },
  { phase: 'four_hourly', label: 'Four hourly', detail: 'Current phase: Four hourly' },
  { phase: 'hourly', label: 'Hourly', detail: 'Current phase: Hourly' },
  { phase: 'fifteen_minute', label: 'Fifteen minute', detail: 'Current phase: Fifteen minute' },
  { phase: 'active_leg', label: 'Active leg', detail: 'Current phase: Active leg' },
...
function getMonitoringPhaseLegendItem(phase) {
  return MONITORING_PHASE_LEGEND.find((item) => item.phase === phase) ?? MONITORING_PHASE_LEGEND[0];
}
...
export function formatMonitoringPhaseCurrentLabel(phase) {
  return getMonitoringPhaseLegendItem(phase).label;
}

export function formatMonitoringPhaseTooltip(phase) {
  const current = getMonitoringPhaseLegendItem(phase);
...
export function computeMonitoringPhase(trip, now = null) {
  ...
  if (nextTiming) {
    ...
    return {
      ...
      currentPhaseLabel: getMonitoringPhaseLegendItem(phase).label,
      ...PHASE_LABELS[phase],
    };
  }
